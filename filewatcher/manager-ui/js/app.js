...
App.controller("mainCtrl", [
    ...
    function($scope, $http, $timeout, Socket) {
		...
		Socket.addEventListener("statuschanged", function(s) {
			$scope.config = s
		}, $scope)
		Socket.connect()

		function init() {
			Socket.sendAPIRequest("status.get").then(function(resp) {
				$scope.config = resp
				$scope.configReady = true
			}, function(reason) {
				console.log("Failed init: "+reason)
			})
		}

		$scope.stopFileWatch = function() {
			Socket.sendAPIRequest("filewatch.stop").then(function(result) {
			}, function (reason) {
				alert(reason)
			})
		}

		$scope.startFileWatch = function() {
			Socket.sendAPIRequest("filewatch.start", {File:$scope.config.File}).then(function(result) {
			}, function (reason) {
				alert(reason)
			})
		}
		...
		$scope.addServer = function() {
			if ($scope.newServer == "") return
			Socket.sendAPIRequest("resultsserver.add", {ServerAddress:$scope.newServer}).then(function(result) {
			}, function (reason) {
				alert(reason)
			})
		}
		...
		$scope.removeServer = function(s) {
			Socket.sendAPIRequest("resultsserver.remove", {ServerAddress:s}).then(function(result) {
			}, function (reason) {
				alert(reason)
			})
		}
